...
import de.jcup.jsoneditor.EclipseUtil;
...
public class HighspeedJSONEditorContentOutlinePage extends ContentOutlinePage implements IDoubleClickListener {
	private static ImageDescriptor IMG_DESC_LINKED = EclipseUtil.createImageDescriptor("/icons/outline/synced.png",
			HighspeedJSONEditorActivator.PLUGIN_ID);
	private static ImageDescriptor IMG_DESC_NOT_LINKED = EclipseUtil
			.createImageDescriptor("/icons/outline/sync_broken.png", HighspeedJSONEditorActivator.PLUGIN_ID);
	...
	public void rebuild(HighspeedJSONScriptModel model) {
		if (model == null) {
			return;
		}
